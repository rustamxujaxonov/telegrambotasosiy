...
async def update_premium_request(request_id: int, **kwargs):
    for k, v in kwargs.items():
        await pool.execute(f"UPDATE premium_requests SET {k} = $1 WHERE id = $2", v, request_id)
...
async def find_match(user_id: int, gender_want: str):
    ...
    row = await pool.fetchrow("""
        ...
        WHERE sq.user_id != $1 AND (sq.gender_want = $2 OR sq.gender_want = 'any') AND ($3 = 'any' OR u.gender = $3)
